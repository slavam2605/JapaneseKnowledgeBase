...
Процессы (как наполнять/разбирать/править/извлекать) — в скиллах `.claude/skills/` (`japanese-cards`,
`process-inbox`, `process-remarks`, `mnn-extract`); они ссылаются на `knowledge/`, не дублируют её.
