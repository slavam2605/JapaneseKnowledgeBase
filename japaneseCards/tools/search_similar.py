...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--deck",
        help="Exact deck to search in. Mutually exclusive with --deck-prefix.",
    )
    parser.add_argument(
        "--deck-prefix",
        help=(
            "Search all decks whose name starts with this prefix, e.g. "
            "'0. Архив::日本語' to search the whole archive at once."
        ),
    )
    parser.add_argument(
        "--query-deck",
        help="Deck containing --note-id. Defaults to --deck / --deck-prefix.",
    )
    parser.add_argument(
        "--db",
        type=Path,
...
def build_text_query(text: str) -> str:
    # bge-m3 needs no instruction prefix; embed the query text as-is.
    return text
...
def search(
    ...
    if deck_prefix is not None:
        deck_clause = "e.deck_name LIKE ? ESCAPE '\\'"
        deck_param = escape_like_prefix(deck_prefix) + "%"
    else:
        deck_clause = "e.deck_name = ?"
        deck_param = deck
...
def main() -> int:
    ...
    if (args.deck is None) == (args.deck_prefix is None):
        print("error: pass exactly one of --deck or --deck-prefix", file=sys.stderr)
        return 1
    ...
    # A query note lives in one exact deck; --deck-prefix only scopes the search target.
    query_deck = args.query_deck or args.deck
    if args.note_id is not None and query_deck is None:
        print("error: with --note-id and --deck-prefix you must pass --query-deck (exact deck of the note)",
              file=sys.stderr)
        return 1
    ...
    print(f"Deck: {args.deck_prefix + '*' if args.deck_prefix else args.deck}")
    ...
    for index, (score, _note_id, row) in enumerate(matches, start=1):
        ...
        deck_tail = str(row["deck_name"]).split("::")[-1]
        print(f"{index:>2}. {score:.4f}  {row['note_id']}  [{deck_tail}]  {front}")
        if back:
            print(f"    {back}")
